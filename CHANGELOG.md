...
- **Lane A 地基**:`pyproject.toml` + 配置层 + Pydantic 知识 Schema(`Evidence`、`CompetitorAnalysis`、`QCResult`)+ Doubao schema `$ref` 内联(`to_doubao_schema`)+ `structured_call` 包装器(validate-retry + 显式 error,Spike A 后切 function-calling/tools 路径,目标 EP `ep-20260514111325-xjmj7` 不支持 `response_format`)。

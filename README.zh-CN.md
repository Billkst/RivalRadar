...
**在线 Demo**:https://rival-radar-seven.vercel.app （前端 Vercel + 后端 Render + Supabase Postgres 持久化;Render 免费档首次访问约 1 分钟冷启动属正常）
...
DATABASE_URL=          # 可选,设 postgres:// 连接串(如 Supabase)即切 Postgres 持久化;不设走本地 SQLite
```
...
| Agent 编排 | LangGraph `StateGraph`(6 节点 collect/analyze/write/qc/decide/finalize + 分支路由 + 闭环重试) |
| LLM | Doubao(字节跳动 ARK 平台,function-calling / tools 路径) |
| 搜索 | Tavily(主)+ Exa(兜底),`FallbackSearch` 自动切换 |
| 存储 | **SQLite / Supabase Postgres 双方言** —— 默认 SQLite(WAL,12 表);设 `DATABASE_URL=postgres://` 即切 Postgres(psycopg3 懒加载);同一套 repository 代码用标准 `ON CONFLICT` 通吃两端。12 表:`runs / evidence / analysis / decisions / insight / qc_result / report / trace / annotations / queries / curation_drops / agent_skills` |
...
在线 Demo = 前端 **Vercel**(Vite 静态)+ 后端 **Render**(FastAPI 常驻容器,单 worker,长连 SSE)+ **Supabase Postgres** 持久化(设 Render 的 `DATABASE_URL` 即接入,容器重启不丢数据)。

完整分步部署(Render / Vercel 配置 + Supabase 接入)见 [`DEPLOY.md`](DEPLOY.md)。
